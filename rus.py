...
class RUSBoost(AdaBoostClassifier):
    ...
    def fit(self, X, y, sample_weight=None, minority_target=None):
        ...
        for iboost in range(self.n_estimators):
            ...
            sample_weight_rus =                 sample_weight[np.where(y != self.minority_target)][X_idx]
            sample_weight_min =                 sample_weight[np.where(y == self.minority_target)]
            ...
            # Combine the weights.
            sample_weight =                 np.append(sample_weight_rus, sample_weight_min).reshape(-1, 1)
